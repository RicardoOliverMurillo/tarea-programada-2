...
public class reporteCitasPaciente extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					reporteCitasPaciente frame = new reporteCitasPaciente();
	...
	public reporteCitasPaciente(int pcedula) {		
		...
		btnBuscar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if((d1!=null) && (d2!=null)){
					if(cbEstados.getSelectedItem()!=""|| cbEspecialidad.getSelectedItem()!=null) {
						JOptionPane.showMessageDialog(null, "No se puede hacer consulta con multiples opciones selecionadas");
					}else{
						citasinfo = new Cita().getCitasFechaPaciente(d1, d2,pcedula);
						setRowValues();
					}
				}else if(cbEstados.getSelectedItem()!="") {
					if( d1!=null || d2!=null ||cbEspecialidad.getSelectedItem()!=null) {
						JOptionPane.showMessageDialog(null, "No se puede hacer consulta con multiples opciones selecionadas");
					}else {
						citasinfo = new Cita().getCitasEstadoPaciente(cbEstados.getSelectedItem().toString(),pcedula);
						setRowValues();
						}
				}else if(cbEspecialidad.getSelectedItem()!=null) {
					if( d1!=null || d2!=null ||cbEstados.getSelectedItem()!="") {
						JOptionPane.showMessageDialog(null, "No se puede hacer consulta con multiples opciones selecionadas");
					}else {
						citasinfo = new Cita().getCitasEspecialidadPaciente(cbEspecialidad.getSelectedItem().toString(),pcedula);
						setRowValues();					}
